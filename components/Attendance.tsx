...
const Attendance: React.FC<AttendanceProps> = ({ isDarkMode, setActiveView, addActivity }) => {
    ...
    <div className={`flex flex-col h-full transition-colors duration-300 relative ${isDarkMode ? 'bg-[#0f172a]' : 'bg-gray-50'}`}>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-6 py-6 space-y-6">
        <div className={`p-4 rounded-3xl flex items-center gap-4 border transition-colors ${isDarkMode ? 'bg-[#1e293b] border-[#334155]' : 'bg-white border-gray-100 shadow-sm shadow-blue-900/5'}`}>
          <div className="w-12 h-12 bg-[#004691] rounded-full flex items-center justify-center text-white font-bold text-sm border-2 border-white/20">
            {MOCK_ACCOUNT.name.split(' ').map(n => n[0]).join('')}
          </div>
          <div className="flex flex-col">
            <h4 className={`font-bold text-sm leading-tight ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>{MOCK_ACCOUNT.name}</h4>
            <p className="text-[10px] text-gray-500 font-bold uppercase mt-0.5 tracking-wider">BCA-992102 • IT Division</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <button 
            ...
            className={`group relative h-32 rounded-[2.5rem] font-bold text-sm overflow-hidden transition-all active:scale-95 flex flex-col items-center justify-center gap-2 shadow-xl border-2 backdrop-blur-xl ${
        ...
        <div className="space-y-4 pt-2 pb-12">
           ...
           <div className={`rounded-[2rem] divide-y shadow-sm border transition-colors overflow-hidden ${
            ...
            <div className="flex justify-between items-center mb-4 px-1">
              <div className="flex items-center gap-2.5">
                <div className={`p-2 rounded-lg ${
                  activeModal === 'izin' ? 'bg-orange-100 text-orange-600' :
            ...
            {activeModal === 'izin' && (
              <form onSubmit={(e) => {
                ...
                <AttendanceField label="Alasan" isDarkMode={isDarkMode}>
                  <textarea required placeholder="Tulis alasan..." className={`w-full py-2.5 px-3.5 rounded-xl text-xs border-2 focus:outline-none focus:border-[#004691] min-h-[70px] resize-none font-semibold ${isDarkMode ? 'bg-[#0f172a] border-[#334155] text-white' : 'bg-white border-gray-100 text-gray-800'}`}></textarea>
                </AttendanceField>
            ...
            {activeModal === 'sakit' && (
              <form onSubmit={(e) => handleModalSubmit(e, 'Sakit')} className="space-y-3.5">
                <AttendanceField label="Tanggal Mulai" isDarkMode={isDarkMode}>
                  <input type="date" required defaultValue={new Date().toISOString().split('T')[0]} className={`w-full py-2.5 px-3.5 rounded-xl text-xs border-2 focus:outline-none focus:border-[#004691] font-semibold ${isDarkMode ? 'bg-[#0f172a] border-[#334155] text-white' : 'bg-white border-gray-100 text-gray-800'}`} />
                </AttendanceField>
                <AttendanceField label="Keterangan" isDarkMode={isDarkMode}>
                  <textarea required placeholder="Gejala singkat..." className={`w-full py-2.5 px-3.5 rounded-xl text-xs border-2 focus:outline-none focus:border-[#004691] min-h-[70px] resize-none font-semibold ${isDarkMode ? 'bg-[#0f172a] border-[#334155] text-white' : 'bg-white border-gray-100 text-gray-800'}`}></textarea>
                </AttendanceField>
                <SubmitButton loading={modalLoading} />
              </form>
            )}
              ...
              <form onSubmit={(e) => handleModalSubmit(e, 'Cuti')} className="space-y-3.5">
                <div className="grid grid-cols-2 gap-2">
                  <AttendanceField label="Mulai" isDarkMode={isDarkMode}>
                    <input type="date" required className={`w-full py-2.5 px-2 rounded-xl text-[10px] border-2 focus:outline-none focus:border-[#004691] font-semibold ${isDarkMode ? 'bg-[#0f172a] border-[#334155] text-white' : 'bg-white border-gray-100 text-gray-800'}`} />
                  </AttendanceField>
                ...
                <AttendanceField label="Keperluan" isDarkMode={isDarkMode}>
                  <textarea required placeholder="Tujuan cuti..." className={`w-full py-2.5 px-3.5 rounded-xl text-xs border-2 focus:outline-none focus:border-[#004691] min-h-[70px] resize-none font-semibold ${isDarkMode ? 'bg-[#0f172a] border-[#334155] text-white' : 'bg-white border-gray-100 text-gray-800'}`}></textarea>
                </AttendanceField>
                <SubmitButton loading={modalLoading} />
              </form>
            )}
...
const AttendanceLogItem: React.FC<{ 
  ...
  status: string;
  location: string;
}> = ({ isDarkMode, date, time, status, location }) => (
  <div className="p-4 flex justify-between items-center active:bg-gray-500/5 transition-colors cursor-pointer group">
    <div className="flex items-center gap-3">
      <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${
        status.includes('Hadir') ? (isDarkMode ? 'bg-green-900/30' : 'bg-green-50') : 
