...
export const Dashboard: React.FC<DashboardProps> = ({ isDarkMode, toggleDarkMode, setActiveView, activities }) => {
  ...
  const userFirstName = MOCK_ACCOUNT.name.split(' ')[0];
  ...
  return (
    <div className={`flex flex-col min-h-full transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a]' : 'bg-[#F8FAFC]'}`}>
      ...
      {/* Header Card */}
      <div className={`px-6 pt-12 pb-6 relative overflow-hidden transition-colors duration-500 flex justify-between items-center rounded-b-[2.5rem] ${
        isDarkMode ? 'bg-gradient-to-r from-blue-900/40 to-indigo-900/40 text-white' : 'bg-gradient-to-r from-[#004691] to-[#00AEEF] text-white'
      }`}>
        ...
        <div className="flex items-center gap-4 z-10">
          <div className={`w-14 h-14 rounded-full border-2 border-white/20 overflow-hidden shadow-md bg-gray-500`}>
            <img src={MOCK_ACCOUNT.avatarUrl} alt="Profile" className="w-full h-full object-cover" />
          </div>
          <div>
            <p className="text-sm font-light text-blue-200">Welcome Back,</p>
            <h1 className="text-white font-black text-xl leading-tight tracking-tight">{MOCK_ACCOUNT.name}</h1>
            <p className="text-xs text-blue-200/80 mt-0.5">{MOCK_ACCOUNT.jobTitle}</p>
          </div>
        </div>
      ...
      <div className="px-6 pt-8 pb-8 space-y-8">
        ...
        <div className="grid grid-cols-4 gap-y-6">
          ...
          <MenuIcon 
            isDarkMode={isDarkMode} 
            icon={<LayoutGrid size={24} className="text-gray-500" />} 
            label="Lainnya" 
          />
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className={`rounded-[2.2rem] overflow-hidden shadow-sm border transition-all ${
            isDarkMode ? 'bg-[#1e293b] border-white/5' : 'bg-white border-gray-100 shadow-blue-900/5'
          }`}>
...
const MenuIcon: React.FC<{ icon: React.ReactNode; label: string; isDarkMode: boolean; onClick?: () => void }> = ({ icon, label, isDarkMode, onClick }) => (
  <button onClick={onClick} className="flex flex-col items-center gap-2 group">
    <div className={`w-14 h-14 rounded-2xl shadow-sm flex items-center justify-center group-active:scale-90 transition-all border ${
      isDarkMode 
