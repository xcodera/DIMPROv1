...
const AIChat: React.FC<AIChatProps> = ({ isDarkMode }) => {
  ...
  return (
    <div className={`flex flex-col h-[calc(100vh-140px)] transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a]' : 'bg-gray-50'}`}>
      {/* Header */}
      <div className={`px-6 py-4 border-b flex items-center gap-3 transition-colors ${isDarkMode ? 'bg-[#1e293b] border-[#334155]' : 'bg-white border-gray-100'}`}>
        ...
        {messages.map((m) => (
          <div key={m.id} className={`flex ${m.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] flex items-start gap-2.5 ${m.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                {/* Message text changed to text-sm (14px) */}
      ...
      {/* Input Area */}
      <div className={`px-6 py-4 border-t transition-colors ${isDarkMode ? 'bg-[#1e293b] border-[#334155]' : 'bg-white border-gray-100'}`}>
